...
function fuckVideo(aid) {
  ...
  // 设置超时处理
  const timeoutDuration = 5000; // 设置超时时间为5000毫秒（5秒）
  const timeoutId = setTimeout(() => {
    updateDiagnosticInfo('<strong style="font-size: 2em; color: orange;">请求超时，请检查网络连接</strong><br>');
    xhr.abort(); // 中止请求
  }, timeoutDuration);
  ...
  xhr.onerror = function () {
    clearTimeout(timeoutId); // 清除超时定时器
    updateDiagnosticInfo('<strong style="font-size: 2em; color: red;">请求发生错误</strong><br>');
  };
