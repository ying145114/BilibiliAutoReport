...
// 查找所有类名为 small-item 的 <div> 元素
const itemNodes = document.querySelectorAll('.small-item');
const urls = [];

// 使用 for...of 循环遍历找到的元素，并提取 srcset URL
for (const itemNode of itemNodes) {
    // 查找第一个具有 srcset 属性的 <source> 元素
    const sourceNode = itemNode.querySelector('source[srcset]');

    if (sourceNode) {
        // 提取 srcset 属性并拼接成完整链接
...
console.log(urls);
// 输出提取到的链接并格式化为期望的字符串
const output = urls.map(url => url.split('@')[0]).join(',');
